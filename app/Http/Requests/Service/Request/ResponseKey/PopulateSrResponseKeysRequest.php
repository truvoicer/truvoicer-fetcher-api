...
class PopulateSrResponseKeysRequest extends FormRequest
{
    ...
    public function rules(SrConfigService $srConfigService): array
    {
        ...
        $responseFormatErrorMsg = sprintf(
            'Response format property/config is not set or valid. %s: %s',
            PropertyType::RESPONSE_FORMAT->value,
            $responseFormatValue
        );

        // Build base fields based on response format
        $fields = match ($responseFormatValue) {
            'json' => DataConstants::REQ_SR_FIELDS_FOR_JSON_POPULATE,
            'xml' => DataConstants::REQ_SR_FIELDS_FOR_XML_POPULATE,
            default => throw new BadRequestHttpException($responseFormatErrorMsg)
        };

        // Create field rules
        $fieldRules = [];
        foreach ($fields as $field) {
            $fieldRules[$field['name']] = ['sometimes', 'string'];
        }
