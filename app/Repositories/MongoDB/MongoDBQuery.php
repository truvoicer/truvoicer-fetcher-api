...
class MongoDBQuery
{
    ...
    {
        return $this->connection->table($this->collection);
    }
    ...
    public function findAll()
    {
        return $this->getResults(
            $this->connection->table($this->collection)
        );
    }

    public function findById(int $id): ?object
    {
        return $this->connection->table($this->collection)->find($id);
    }
    ...
    private function buildQuery(): void
    {
        ...
        if (count($this->whereGroups)) {
            foreach ($this->whereGroups as $index => $whereGroup) {
                $this->builder = $this->buildWhereGroups($this->builder, $whereGroup, $index === 0);
            }
        }
        ...
        $this->builder->orderBy($this->sortField, $this->orderDir);
